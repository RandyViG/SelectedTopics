...
def getTf_Idf( topics , M):
    tf_idf = [ ]
    k = 1.2
    for t in topics:
        probability = (k+1) / (t[1]+k) * math.log(M/t[1])
        tf_idf.append( ( t[0] , probability ) )
...
if __name__ == "__main__":
    ...
    tf_Idf = getTf_Idf( topics , M )

    tf_Idf = sorted(tf_Idf,key=operator.itemgetter(1),reverse=True)

    fv=open('ProbabilityFrequencyTopic.txt','w')
    for t in tf_Idf:
        fv.write( '{:30}{:30}\n'.format( t[0],str(t[1]) ) )
